...
print("Exercícios 1.1.")

paises = []

for dado_de_usuario in propaganda_online:
    if dado_de_usuario['idade'] > 30:
        paises = dado_de_usuario['pais']
        print(paises)
        
print('\n') 
#1.2 Crie uma lista chamada leads com a renda dos usuários que clicaram na propaganda.
print("Exercícios 1.2.") 
leads = []
for dado_de_usuario in propaganda_online:
    try:
        if dado_de_usuario['clicou_no_ad'] > 0:
            leads = dado_de_usuario['renda_area']
            print(leads)
    except KeyError:
            x = "Dicionario não tem a chave do usuario com quantidade de clicks = KeyError"

print('\n') 

#1.3. Crie uma lista chamada cidades com a cidade dos usuários que passaram mais de 70 segundos no site.
print("Exercícios 1.3.") 
cidades = []
for dado_de_usuario in propaganda_online:
    try:    
        if dado_de_usuario['tempo_gasto_site'] > 70:
            cidades = dado_de_usuario['cidade']  
            print(cidades)
    except TypeError:
            x = "O tempo gasto do usuário está vazio"
